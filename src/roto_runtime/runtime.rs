...
pub fn create_runtime() -> Result<roto::Runtime, String> {
    ...
    //rt.register_method::<BmpMsg<Bytes>, _, _>("aspath_contains",
    //bmp_aspath_contains)?;
    ...
    fn bmp_aspath_origin(msg: *const BmpMsg<Bytes>, to_match: Asn) -> bool {
        let msg = unsafe { &*msg };
    ...
    //rt.register_method::<Log, _, _>("log_matched_community",
    //log_community)?;
...
fn contains_community(
    ...
    if let Some(mut iter) = bgp_update.communities().ok().flatten() {
        iter.any(|c| Community::Standard(to_match.into()) == c)
    } else {
        false
    }
}
