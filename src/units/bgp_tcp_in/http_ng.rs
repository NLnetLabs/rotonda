...
/// Return all BGP sessions from ingress register
fn all_bgp_sessions(state: &State<ApiState>) -> Vec<OwnedIdAndInfo> {
    state.ingress_register.search(QueryFilter {
        ingress_type: Some(IngressType::Bgp),
        ..Default::default()
    })
}
