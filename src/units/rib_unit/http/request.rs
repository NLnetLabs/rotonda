...
impl PrefixesApi {
    async fn handle_prefix_query(
        ...
        let prefix = Prefix::from_str(req_path.strip_prefix(self.http_api_path.as_str()).unwrap()) // SAFETY: unwrap() safe due to starts_with() check above
        ...
        let includes = Self::parse_include_param(&params, self.query_limits.clone(), prefix)?;
    ...
    fn parse_filter_params(params: &QueryParams) -> Result<Filters, String> {
            ...
            .map(MatchedParam::value)
        {
            ...
            Some(other) => return Err(format!("Unknown filter_op value '{}'", other)),
...
fn extract_filter_kind(filter: MatchedParam) -> Result<FilterKind, String> {
    let extracted_filter = match filter {
        ...
        MatchedParam::Family("community", v) => match Community::from_str(v) {
