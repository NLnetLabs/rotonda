...
pub(super) struct RouteDbRouteEvpn<'a>(pub Route<'a>);
...
crate::genoutput_json!(RouteDbRouteEvpn<'_>);
...
impl Serialize for RouteDbRouteEvpn<'_> {
    fn serialize<S>(&self, s: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        state.serialize_field(
            "nlri",
            <&routecore::bgp::message_ng::nlri::EvpnNlri>::try_from(nlri)
                .unwrap(),
        )?;
        state.serialize_field("pathAttributes", &prepped_attrs)?;
        state.end()
